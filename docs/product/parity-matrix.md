...
| `coding-agent.mode.rpc` | `internal/cli/run.go` `RunRPC`, `internal/rpc/` | `TestACommandWithoutAnIdIsRefused`, `TestAPromptsResponseAndItsEventsShareOneOrder`, `TestAbortEndsARunningPromptAndTheStreamSaysAborted`, `TestSteerAndFollowUpReachTheRunInFlightAndNothingElse`, `TestASecondPromptWhileOneRunsIsBusy`, `TestSwappingTheConversationOrModelWaitsForTheRun`, `TestTheRecordedConversationCanBeWalkedForkedAndLeft` (a real file store: fork, switch back, new), `TestTheTreeAndTheEntriesAreTwoViewsOfOneRecord`, `TestTheOrderIsTheWriteOrderUnderConcurrency` (confirmed to fail 5/5 with `seq` allocated outside the write lock), `TestAnUnknownVerbAndAnUnbuiltOneAreDifferentAnswers`, `TestAProviderFailureAtStartKeepsItsClassification` | partial — the channel ships per ADR-0010 (D-17): required ids, responses on the stream's one `seq`, typed failures. **18 of Pi's 32 commands answer natively**: `prompt`, `abort`, `steer`, `follow_up`, `get_state`, `get_messages`, `get_session_stats`, `get_last_assistant_text`, `set_session_name`, `get_tree`, `get_entries`, `fork`, `clone`, `switch_session`, `new_session`, `set_model`, `compact`, `get_commands`. Anything that swaps the conversation or the model is `busy` while a prompt runs; a run kept in memory answers `unavailable` to the store-backed ones. The 14 left fail with a typed kind that separates unknown from unbuilt, each tracked (#27 export, #28 bash, #30 models, #36 thinking, #39 retry, #40 queue modes, #41 auto-compaction; `get_fork_messages` is the fork picker's listing and waits on #28) |
