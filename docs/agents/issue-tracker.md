...
[`iamclancyliang/pi-go`](https://github.com/iamclancyliang/pi-go). Use the `gh` CLI for all
operations.

**Write access verified**: the repo is owned by `iamclancyliang` with `admin`, `push`, and `triage`
permissions, so creating issues, applying labels, and assigning all work here. (This is unlike the
upstream `earendil-works/pi` study checkout, which is read-only and therefore uses a local markdown
tracker instead.)

## Conventions

- **Create an issue**: `gh issue create --title "..." --body-file <file>`. Write multi-line bodies
  to a temp file rather than passing them inline.
...
- **Close**: `gh issue close <number>`. If the close needs a comment, post it first with
  `gh issue comment <number> --body-file <file>`, then close — `gh issue close --comment` only
  accepts inline text. To close via commit, include `fixes #<n>` / `closes #<n>`, repeating the
  keyword per issue (`closes #1, closes #2`), since a shared keyword only closes the first.
...
- **Comment / label / close**: `gh pr comment --body-file`, `gh pr edit --add-label`/
...
- **Blocking**: GitHub's **native issue dependencies**. Add an edge with
  `gh api --method POST repos/iamclancyliang/pi-go/issues/<child>/dependencies/blocked_by -F issue_id=<blocker-db-id>`,
  where `<blocker-db-id>` is the blocker's numeric **database id**
  (`gh api repos/iamclancyliang/pi-go/issues/<n> --jq .id`, _not_ the `#number` or `node_id`).
